	...
	public static String longestPrefix(String[] s) {
		...
		for(int i = 1; i < s.length; i++) { // traverse through the other Strings in the array
			while(s[i].indexOf(prefix) != 0) { // cut the prefix down until the two strings are EQUAL they will be ZERO
				prefix = prefix.substring(0, prefix.length() - 1); // keep subtracting letters from the prefix until both strings are = 0 meaning they are the same. Which is the longest prefix
			}
		}
		return prefix;
	}
